 ...
 * 角色的家用 ★ 标注（来自 character.homeNodeId）。
 */
function describeMapGraph(
  nodes: MapNode[],
  ...
  const render = (n: MapNode, depth: number): void => {
    ...
    const home = n.id === homeNodeId ? " ★ 你的家" : "";
    treeLines.push(
      `${indent}- ${n.name} [${n.id}]（${n.privacy}${tagPart}）${cost}${home}`,
    );
  ...
  let out = `当前世界地图（缩进=父子；★=你的家；⏱N=进入需 N 小时；可用于多步路径规划，target_node_id 用方括号内的 id）：\n${treeLines.join("\n")}`;
...
export function buildUserPrompt(args: {
  ...
  if (facts.homeNodeName) {
    lines.push(`你的常规作息：22:00–06:00 在 ${facts.homeNodeName} 休息。`);
  } else {
