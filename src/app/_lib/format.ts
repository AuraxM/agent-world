...
/** tick → "2026/05/02 周六" date with weekday */
export function formatDay(epoch: number, tick: number): string {
  ...
  const dd = String(d.getDate()).padStart(2, "0");
  const w = WEEKDAY_NAMES[d.getDay()];
  return `${yyyy}/${mm}/${dd} 周${w}`;
}

/** tick → "2026年5月3日 星期六 14:00" full Chinese date with weekday */
export function formatDateTimeCN(epoch: number, tick: number): string {
  const d = tickToDate(epoch, tick);
