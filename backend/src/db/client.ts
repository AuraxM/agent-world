 ...
 *
 * Includes self-healing ensureColumns that creates missing tables/columns
 * on first open, so both persistent and :memory: databases work without
 * requiring a separate migrate() call.
 */
...
const THINK_SESSIONS_TABLE_MIGRATION: Array<[string, string]> = [
  ["think_sessions", `CREATE TABLE IF NOT EXISTS think_sessions (
    id TEXT NOT NULL,
    world_id TEXT NOT NULL REFERENCES worlds(id) ON DELETE CASCADE,
    payload_json TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch() * 1000),
